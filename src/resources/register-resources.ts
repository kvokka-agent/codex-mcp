...
function buildGotchasText(): string {
  return [
    ...
    "- **Check frequency guidance**: one `waitMs` call costs one round trip and covers the whole wait. Without it, adapt the interval to the task: for `running` sessions start at 2 minutes and go up for long tasks; only check often (~1s) while `waiting_approval`.",
    "",
    "## Approval behavior",
    "",
...
function buildQuickstartText(): string {
  return [
    ...
    "- `waitMs` holds the call until something you act on happens, so one call covers a long stretch of work.",
...
function buildDelegationGuideText(): string {
  return [
    ...
    'For long tasks, omit `waitForResult`. Use `codex_check(action="poll", waitMs=120000)`: it returns when the status changes, an action arrives or the turn ends.',
    "",
    "## Effort selection",
