...
function buildConfigGuideText(): string {
  return [
    ...
    `- \`codex_check.maxEvents\`: default \`${POLL_DEFAULT_MAX_EVENTS}\` for \`poll\` (minimum \`${POLL_MIN_MAX_EVENTS}\`), \`${RESPOND_DEFAULT_MAX_EVENTS}\` for \`respond_permission\` and \`respond_user_input\`.`,
...
function buildGotchasText(): string {
  return [
    ...
    `- \`maxEvents\` is a top-level \`codex_check\` field; \`pollOptions\` rejects it and every other key it does not declare.`,
    `- Every response mode caps a per-event \`delta\` and marks the cut one with \`deltaTruncated\`: minimal ${RESPONSE_MODE_DELTA_LIMITS.minimal}, delta_compact ${RESPONSE_MODE_DELTA_LIMITS.delta_compact}, full ${RESPONSE_MODE_DELTA_LIMITS.full} characters.`,
    ...
    "- `progress` normalizes the current phase, pending action count, last observed method, and the token totals the backend has reported.",
    "- Fine-grained stream semantics are in `events[].data.method` (for example command output delta, reasoning delta, turn updates).",
    '- Retryable interruptions surface as `progress` with `method="codex-mcp/reconnect"`, carrying the `willRetry` flag and the `error` behind it; the backend sends no attempt counter.',
    "- `willRetry=false` moves the session to the error path; while it is true, keep polling normally.",
    "",
