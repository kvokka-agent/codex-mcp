...
function buildGotchasText(): string {
  return [
    ...
    "- **A caller nobody can see**: `notifications/progress` reaches the MCP client, and a client renders it under the call it made itself, so a call made inside a subagent shows the person watching nothing while it runs. Such a caller keeps the line of every round — the new `progress.activity`, or the standing one with `progress.activityStandingMs` in minutes — and returns them all in what it reports, which is the only path they travel.",
    "",
    "## Approval behavior",
    "",
