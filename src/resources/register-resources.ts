...
function buildGotchasText(): string {
  return [
    ...
    "- **A caller nobody can see**: `notifications/progress` reaches the MCP client, and a subagent's client shows it to nobody watching the subagent. A caller in that position asks for a `waitMs` of about 60000 instead and writes `progress.activity` out itself after each round, which is what puts the turn's own words in front of the person waiting.",
    "",
    "## Approval behavior",
    "",
...
function buildQuickstartText(): string {
  return [
    ...
    "- `waitMs` holds the call until something you act on happens, so one call covers a long stretch of work. Ask for more than the task can take: the server holds the call for as long as the MCP client tolerates and answers just inside that.",
...
function buildDelegationGuideText(): string {
  return [
    ...
    `For long tasks, omit \`waitForResult\`. Use \`codex_check(action="poll", waitMs=${MAX_LONG_POLL_WAIT_MS})\`: it returns when the status changes, an action arrives or the turn ends, and otherwise holds the call for as long as the MCP client allows.`,
    "",
    "## Effort selection",
