...
function buildGotchasText(): string {
  return [
    ...
    "- **A caller nobody can see**: `notifications/progress` reaches the MCP client, and a client renders it under the call it made itself, so a call made inside a subagent shows the person watching nothing. Drive the loop where your own lines reach the person waiting, and write `progress.activity` out after every round.",
    "",
    "## Approval behavior",
    "",
...
function buildQuickstartText(): string {
  return [
    ...
    "",
    "The line you write between two polls is the whole of it. A client renders `notifications/progress` under the call that asked for it — the server sends one per activity line and a heartbeat every 30s while a poll is held — but only for the caller that made the call, so a poll made inside a subagent shows the person watching nothing. Drive the loop where your own lines are read.",
    "",
    "## Notes",
    "",
