...
- **The plugin drives Codex from the thread the person is reading.** The `codex` subagent and the `PreToolUse` hook that kept the Codex tools inside it are gone, replaced by a `codex` skill. A subagent's tool calls are rendered to nobody: its own lines never reach the person waiting, and `notifications/progress` sent under its call reach the MCP client and stop there. What the person actually saw was a token counter going up for as long as the turn ran. The skill starts one session per task, polls every running session in one message in rounds of 300000ms, and writes one line per round — `codex: <activity>`, or `codex: <activity> — 15 min` when the turn is still on it.
