...
A turn that was in flight ends with the process that drove it. The fresh one
reads the state directory back — its stderr reports how many sessions it
recovered — and reaps the child processes the old one left behind.
...
That is the CI gate in one command: lint, format, types, unit tests, build, and
then the two scripts that launch the built server — `check:stdio` proves stdout
stays free of anything but JSON-RPC before the handshake, `smoke:mcp` proves the
startup path brings up the five tools and the resources. Vitest mocks the child
process and reaches neither. Both scripts accept `--npx` to run the published
package instead of `dist/`, and take a command after `--`.
...
`events.jsonl` is the transcript of a turn; `codex_check` never returns those
events, so this file is where a turn that went wrong is read.

**Environment variables.** `CODEX_MCP_PATH` and `CODEX_MCP_COMMAND` select the
codex binary, `CODEX_MCP_MODE` forces `app-server` or `exec` instead of probing.
The `codex-mcp:///config` resource lists all of them with their defaults; read
it from a session rather than from a copy here.
